<div class="p-2 sm:p-4 lg:p-8">
    <x-filament-panels::page>
        <x-filament::section class="overflow-hidden border-none shadow-none bg-white dark:bg-gray-900 print:p-0">
            {{-- Header Section --}}
            <div class="flex flex-col md:flex-row justify-between items-start gap-8 border-b border-gray-100 dark:border-gray-800 pb-8">
                <div class="flex items-center gap-5">
                    @if(file_exists(public_path('asset/images/logo/gas 200.png')))
                        <img src="{{ asset('asset/images/logo/gas 200.png') }}" class="w-20 h-20 object-contain print:w-16" alt="Logo">
                    @endif
                    <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mt-10">
                <div class="space-y-3">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 p-2 m-2 rounded-xl border border-gray-100 dark:border-gray-700">
                        <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $this->getRecord()->customer?->name??'عميل' }}</p>
                ...
                <div class="flex flex-col md:items-end justify-end space-y-2 m-2">
                    ...
                    <div class="flex justify-between w-full md:w-64 border-b border-gray-50 dark:border-gray-800 pb-1">
                        <span class="text-gray-500 text-bold">{{ trans('filament-invoices::messages.invoices.view.issue_date') }}:</span>
                        <span class="font-medium">{{ $this->getRecord()->created_at->toDateString() }}</span>
                    </div>
                    <div class="flex justify-between w-full md:w-64">
                        <span class="text-gray-500 text-bold">{{ trans('filament-invoices::messages.invoices.view.status') }}:</span>
                ...
                <table class="w-full text-right border-collapse">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-white/5 text-gray-500 dark:text-gray-400 text-xs uppercase font-bold">
                            ...
                            <th class="px-6 py-4 text-center">{{ trans('filament-invoices::messages.invoices.view.qty') }}</th>
                            <th class="px-6 py-4 text-center">{{ trans('filament-invoices::messages.invoices.view.price') }}</th>
                            <th class="px-6 py-4 text-center">{{ trans('filament-invoices::messages.invoices.view.discount') }}</th>
                            <th class="px-6 py-4 text-left">{{ trans('filament-invoices::messages.invoices.view.total') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        @foreach ($this->getRecord()->items as $item)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td class="px-6 py-5 text-center text-gray-600 dark:text-gray-400">{{ number_format($item->price, 1) }}</td>
                                <td class="px-6 py-5 text-center text-red-500">{{ number_format($item->sub_discount * $item->qty, 1) }}</td>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10 p-6 bg-gray-50 dark:bg-white/5 rounded-2xl">
                <div>
                    @if($this->getRecord()->notes)
                        <h4 class="text-xs font-bold uppercase text-gray-400 mb-2">{{ trans('filament-invoices::messages.invoices.view.notes') }}</h4>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-semibold">{{ number_format($this->getRecord()->total + $this->getRecord()->discount - $this->getRecord()->shipping - $this->getRecord()->install, 1) }} {{ $this->getRecord()->currency }}</span>
                    </div>
                    ...
                    <div class="flex justify-between pt-3 border-t border-gray-200 dark:border-gray-700">
                        <span class="text-lg font-bold">{{ trans('filament-invoices::messages.invoices.view.total') }}</span>
                        <span class="text-2xl font-black text-primary-600 dark:text-primary-400">
                            {{ number_format($this->getRecord()->total, 1) }} <small class="text-xs">{{ $this->getRecord()->currency }}</small>
                        </span>
                    </div>
