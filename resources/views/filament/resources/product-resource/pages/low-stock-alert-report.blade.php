                        ...
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <div class="flex items-center">
                                ...
                                <div class="mr-4">
                                    ...
                                    <p class="text-2xl font-bold text-yellow-900">{{ $mediumCount }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($allLowStock as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $item->branch_name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="px-2 py-1 rounded text-xs {{ $item->product_status == 'new' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                                {{ $item->product_status == 'new' ? 'جديد' : 'مستعمل' }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold {{ $item->quantity < 5 ? 'text-red-600' : 'text-yellow-600' }}">
                                            {{ $item->quantity }}
                                        </td>
                                    </tr>
                                @endforeach
